...
let items = []
let page = 1
let Url = 'https://kkapi-open-kappa.vercel.app/api/ispeak?author=64887625bad7c760321c3d03&page=' // 记住替换为你的API链接
...
function getNew() {
    ...
    fetch(Url + page).then(res => res.json()).then((res) => {
        ...
        nowNum += items.length
        if (page == 1) {
            document.querySelector('.bb-info').innerHTML = '<svg style="width:1.20em;height:1.20em;top:5px;fill:currentColor;overflow:hidden;position:relative"><use xlink:href="#icon-chat"></svg> Cằn nhằn của Quản trị viên(' + total + ')'
        }
        page += 1
    }).then(() => {
        bb();
        if (nowNum < total) {
            document.getElementById('bibi').innerHTML += '<button id="more" onclick="getNew()">More</button>'
        }
