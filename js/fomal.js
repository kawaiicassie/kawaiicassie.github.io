...
function showWelcome() {
  ...
  if (date.getHours() >= 5 && date.getHours() < 11)
    timeChange = "<span>Chào buổi sáng</span>，chúc bạn một ngày tốt lành!";
  else if (date.getHours() >= 11 && date.getHours() < 13)
    timeChange = "<span>Mặt trời lên đỉnh</span>，làm việc thật chăm chỉ nha!";
  else if (date.getHours() >= 13 && date.getHours() < 15)
    timeChange = "<span>Chào buổi trưa</span>，nhớ ăn rồi nghỉ ngơi đó!";
  else if (date.getHours() >= 15 && date.getHours() < 16)
    timeChange =
      "<span>Chào buổi chiều</span>，chuẩn bị đồ thật đẹp để đi chơi nè!";
  else if (date.getHours() >= 16 && date.getHours() < 19)
    timeChange = "<span>Hoàng hôn ló dạng!</span>";
  else if (date.getHours() >= 19 && date.getHours() < 24)
    timeChange =
      "<span>Chào buổi tối</span>，vui chơi thoải mái với bạn bè nè!";
  ...
  try {
    ...
    document.getElementById(
      "welcome-info"
    ).innerHTML = `<b><center>🎉 Chào mừng bạn 🎉</center>&emsp;&emsp;Welcome to <span style="color:var(--theme-color)">${pos}</span> KawaiiCassie🌸，${timeChange}Bây giờ là <span style="color:var(--theme-color)">${dist}</span> km, địa chỉ IP là: <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
    ...
    new Vue({
      data: function () {
        this.$notify({
          ...
          message: "Bây giờ bạn có thể chia sẻ liên kết với bạn bè!",
          position: "top-left",
          offset: 50,
          showClose: true,
          type: "success",
  ...
  if (
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Bắt được bạn đang dùng phím tắt! 😜",
            message:
              "Bạn đang tìm gì ở trang này? Nếu cần quá có thể liên hệ mình gửi cho nhé! 🍑",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "warning",
  ...
  if (localStorage.getItem("mouse") == "on") {
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Chuyển chế độ chuột phải thành công 🍔",
            message:
              "Nút chuột phải hiện tại đã được khôi phục về mặc định của hệ thống!",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "success",
      ...
      new Vue({
        data: function () {
          this.$notify({
            title: "Chuyển chế độ chuột phải thành công 🍔",
            message:
              "Nút chuột phải hiện tại đã được thay thế bằng kiểu do trang web chỉ định!",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "success",
...
function createtime1() {
  var grt = new Date("05/28/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
  now1.setTime(now1.getTime() + 250);
  var days = (now1 - grt) / 1000 / 60 / 60 / 24;
  var dnum = Math.floor(days);

  var ascll = [`Chào mừng đến KawaiiCassie🌸`, "©2023 By Cassie"];
...
function createtime2() {
  var ascll2 = [
    `NCC2-036`,
    `Gọi thành công camera trước chụp ảnh`,
    `Photo captured: `,
    ` 🤪 `,
  ];
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ owered by KawaiiCassie🌸 %c Chào mừng bạn đến KawaiiCassie🌸",
  ...
  setTimeout(
    console.log.bind(
      console,
      "%c W23-12 %c Hệ thống đã phát hiện ra rằng bạn đã mở bảng điều khiển",
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c S013-782 %c bạn hiện đang bị giám sát",
    ...
    setTimeout(() => {
      new Vue({
        data: function () {
          this.$notify({
            title: "Tắt đèn 🌙",
            message: "Hiện đã chuyển sang chế độ ban đêm thành công!",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "success",
    ...
    setTimeout(() => {
      new Vue({
        data: function () {
          this.$notify({
            title: "Bật đèn 🌞",
            message: "Hiện đã chuyển thành công sang chế độ ban ngày!",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "success",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| KawaiiCassie🌸")
    ...
    new Vue({
      data: function () {
        this.$notify({
          ...
          message:
            "Bây giờ bạn có thể chia sẻ trực tiếp với bạn bè của mình bằng cách dán nó!",
          position: "top-left",
          offset: 50,
          showClose: true,
          type: "success",
...
function createtime() {
  ...
  var start = new Date("07/01/2023 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6); // 天文单位
  // 网站诞生时间
  var grt = new Date("05/28/2023 00:00:00");
...
if (
  ...
  window.localStorage.getItem("fpson") == "0"
) {
  var rAF = (function () {
    return (
      window.requestAnimationFrame ||
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">Quá thấp 🤢</span>`;
      } else if (fps <= 15) {
        var kd = `<span style="color:red">Thấp 😖</span>`;
      } else if (fps <= 25) {
        var kd = `<span style="color:orange">Trung bình 😨</span>`;
      } else if (fps < 35) {
        var kd = `<span style="color:#9338e6">Không mượt 🙄</span>`;
      } else if (fps <= 45) {
    ...
    new Vue({
      data: function () {
        this.$notify({
          title: "Chào mừng 🍒",
          message:
            "Chào bạn đến với Blog của mình ✨ Trang này có nhiều chức năng lắm nên hãy khám phá nhé, chúc bạn một ngày tốt lành! (｡･∀･)ﾉ",
          position: "top-left",
          offset: 50,
          showClose: true,
          type: "success",
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "Patrick Hand SC");
}
...
function setFont(n) {
  ...
  if (n == "default") {
    ...
    document.body.style.fontFamily =
      "var(--global-font),-apple-system, IBM Plex Mono, monosapce, sans-serif";
  }
  try {
    setFontBorder();
  } catch (err) {}
...
function setTrans() {
  ...
  target.innerHTML = "Độ trong (0%-100%): " + newTransNum + "%";
...
function setBlurNum() {
  ...
  target.innerHTML =
    "Bán kính làm mờ (Khi đã bật làm mờ 0px-100px): " + newBlur + "px";
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://cdn.jsdelivr.net/gh/kawaiicassie/CDN@main/uploads/5cm1s-3-1.png);
    --darkmode-bg:url(/img/15.png);
    --mobileday-bg: url(/img/gallery/mobile/2.webp);
    --mobilenight-bg: url(/img/gallery/mobile/4.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
      ...
      new Vue({
        data: function () {
          this.$notify({
            ...
            message: "Chuyển nền tùy chỉnh thành công!",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "success",
      ...
      new Vue({
        data: function () {
          this.$notify({
            ...
            message: "Liên kết bạn nhập bị sai, hãy thử lại!",
            position: "top-left",
            offset: 50,
            showClose: true,
            type: "warning",
...
<br>
<center><p><button onclick="reset()" style="background:linear-gradient(to right, #654ea3, #eaafc8);display:block;width:40%;padding:15px 0;border-radius:30px;color:white;font-size:1.1em;"><i class="fa-solid fa-arrows-rotate"></i>&nbsp;Khôi phục thiết lập mặc định</button></p></center>

<h2>1. Tuỳ chọn hiển thị</h2>

<div class="transValue" style="font-weight:bold;padding-left:10px">Độ mờ (0%-100%): ${curTransNum}%</div>
<div class="range">
...
<div class="blurValue" style="font-weight:bold;padding-left:10px">Bán kính làm mờ (Khi bật làm mờ 0px-100px): ${curBlur} px</div>
<div class="range">
...
<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Hiệu ứng sao băng (Chế độ ban đêm) </div><input type="checkbox" id="universeSet" onclick="setUniverse()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> Đèn Neon (Chế độ ban đêm) </div><input type="checkbox" id="lightSet" onclick="setLight()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Hiệu ứng Blur (Tiêu thụ hiệu suất) </div><input type="checkbox" id="blur" onclick="setBlur()">
  <div class="content-text" style="font-weight:bold; padding-left:20px"> Thanh bên (Bật theo mặc định) </div><input type="checkbox" id="rightSideSet" onclick="toggleRightside()">
</div>

<div class="content" style="display:flex">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Theo dõi FPS (Refresh) </div><input type="checkbox" id="fpson" onclick="fpssw()">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> Hiệu ứng tuyết rơi (Chế độ ban ngày) </div><input type="checkbox" id="snowSet" onclick="setSnow()">
</div>


<h2>2. Cài đặt phông chữ</h2>
<div class="note warning modern"><p>Phông chữ miễn phí phi thương mại chỉ dành cho mục đích sử dụng cá nhân mà không được phép. Trang web này hoàn toàn phi thương mại và phi lợi nhuận, thường được sử dụng cho mục đích học tập và liên lạc cá nhân, nếu có bất kỳ hành vi vi phạm nào, vui lòng liên hệ với quản trị viên web để xóa nó, xin cảm ơn! —— Gửi chủ sở hữu bản quyền.</p>
</div>
<p id="swfs">
<a class="swf" id="swf_default" href="javascript:;" rel="noopener external nofollow" style="font-family:-apple-system, IBM Plex Mono, monosapce, sans-serif;!important;color:black" onclick="setFont('default')">Mặc định</a>
<a class="swf" id="swf_Roboto" href="javascript:;" rel="noopener external nofollow" style="font-family:'Roboto'!important;color:black" onclick="setFont('Roboto')">Roboto</a>
<a class="swf" id="swf_SegoeUI" href="javascript:;" rel="noopener external nofollow" style="font-family:'Segoe UI'!important;color:black" onclick="setFont('Segoe UI')">Segoe UI</a>
<a class="swf" id="swf_PatrickHandSC" href="javascript:;" rel="noopener external nofollow" style="font-family:'Patrick Hand SC'!important;color:black" onclick="setFont('Patrick Hand SC')">Patrick Hand SC</a>
</p>

<h2>3. Cài đặt màu chủ đề</h2>
...
<h3>7. Nền ngẫu nhiên</h3>
<details class="folding-tag" cyan><summary> Xem nền ngẫu nhiên </summary>
              <div class='content'>
              <div class="bgbox"><a href="javascript:;" rel="noopener external nofollow" style="background-image:url(https://minimalistic-wallpaper.demolab.com/?random)" class="imgbox" onclick="changeBg('url(https://minimalistic-wallpaper.demolab.com/?random)')"></a></div>
              </div>
            </details>

<h3>8. Tuỳ chỉnh nền</h3>
...
<br>
<center><div style="font-size:1.2em;color:var(--theme-color);font-weight:bold;">------ Hết rồi ( •̀ ω •́ )y ------</div></center>
<br>
...
var newYear = () => {
  ...
  if (!document.querySelector("#newYear")) return;
  // 新年时间戳 and 星期对象
  let newYear = new Date("2024-02-19 00:00:00").getTime() / 1000,
    week = {
      0: "Thứ hai",
      1: "Thứ ba",
      2: "Thứ tư",
  ...
  function time() {
    ...
    if (second < 0) {
      ...
      document.querySelector("#newYear .newYear-time").innerHTML =
        '<span class="happyNewYear">Chúc mừng năm mới!</span>';
    } else {
      // 大于0则还未过年
      ...
      if (second > 86400) {
        document.querySelector(
          ...
          second / 86400
        )}<span class="unit">ngày nữa ✨</span></span>`;
      } else {
        // 小于一天则使用时分秒计时。
        let h = nol(parseInt(second / 3600));
        second %= 3600;
...
function catalogActive() {
  ...
  if ($list) {
    ...
    $list.scrollLeft =
      $catalog.offsetLeft -
      $list.offsetLeft -
      ($list.offsetWidth - $catalog.offsetWidth) / 2;
  }
}
catalogActive();
// 切换热评
function switchCommentBarrage() {
  ...
  document.getElementById("comment-barrage").style.display =
    flag === "false" ? "block" : "none";
  // 本地缓存一天，刷新或切换页面时仍 隐藏或显示 热评。
  window.localStorage.setItem(
    "commentBarrageDisplay",
    flag === "false" ? "undefined" : "false",
    86400000
  );
}
